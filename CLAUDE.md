...
- **Scheduled Task Coolify `curl: not found`** : Coolify exécute les Scheduled Tasks **dans** le conteneur de l'app (docker exec), et `node:20-alpine` n'embarque pas curl. Symptôme : "Job permanently failed after 1 attempts: sh: curl: not found" et les publications programmées ne partent pas. Fix : `apk add --no-cache curl` dans le Dockerfile (fait), ou utiliser `wget -qO- --header="Authorization: Bearer $CRON_SECRET" <url>` dans la commande de la tâche. La tâche n'a en réalité JAMAIS fonctionné (tous les runs failed depuis sa création le 30 juin 2026) — les publications "automatiques" antérieures étaient des clics manuels sur Publier.
- **Site public figé malgré les publications (`force=false`)** : symptôme = un dossier passe bien en `publie` en base (vérifiable en SQL) mais n'apparaît jamais sur `www.lenoncote.fr`, dont le `last-modified` ne bouge plus. Cause : `COOLIFY_DEPLOY_WEBHOOK_URL` finissait par `&force=false`. Coolify indexe son cache d'image sur le **SHA git** ; le contenu du site venant de Supabase (pas de git), le SHA ne change pas d'une publication à l'autre → log `No build configuration changed & image found ... Build step skipped` → rolling update sur la **vieille image**. Le deploy manuel dans l'UI skippe pareil. Fix : `&force=true` dans la webhook (+ Redeploy DASH-EI). Pour débloquer un site déjà figé : dans Coolify, flèche ▾ à côté de Deploy → **Force rebuild (without cache)**. Cassé vers le 12 août 2026 (probable auto-update Coolify qui a changé le traitement de `force=false`).
